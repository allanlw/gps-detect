...
function findEXIFinJPEG(file) {
	...
	while (offset < length) {
		...
		if (marker == 0xE1) {
			...
			offset += 4;
		} else if (marker >= 0xD0 && marker <= 0xDA) { // markers which do NOT have a two-byte size
			// we're not smart enough to traverse these
			return null;
		// You can have as many 0xFFs as you want
		} else if (marker == 0xFF) {
			offset += 1;
		} else { // normal data block with 2-byte variable size
